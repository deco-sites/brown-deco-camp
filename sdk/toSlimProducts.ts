...
export const toSlimProducts = (products: Product[]) => {
  return products.map((product) => ({
    ...
    name: product.name ?? "",
    description: product.description,
    formatHighPrice: formatPrice(product.offers?.highPrice, product.offers?.priceCurrency),
    formatLowPrice: formatPrice(product.offers?.lowPrice, product.offers?.priceCurrency),
